...
public class LlistaAccessos implements InLlistaAccessos, Serializable{
     ...
     */
     ...
     public String llistarAccessos(boolean estat) throws ExcepcioEstacio{
        ...
        if(llistaAccessos.isEmpty()) throw new ExcepcioEstacio("No hi ha cap access registrat.");
        else{
            for(Acces acc : llistaAccessos){
                if(acc.isEstat() == estat) accessos.append(acc.toString());
            }
        }
     ...
     public void actualitzaEstatAccessos() throws ExcepcioEstacio{
         ...
         if(!llistaAccessos.isEmpty()){
             for(Acces acc : llistaAccessos){
                 ...
                 ArrayList llistaVies = llista.getLlistaVies();
                 Iterator<Via> it = llistaVies.iterator();
                 while(it.hasNext()){
                    if(it.next().isEstat()) acc.setEstat(true);
                 } 
             }
         }
         else{
     ...
     */
     ...
     public float calculaLongitudAccessosNivell() throws ExcepcioEstacio{
         ...
         if(!llistaAccessos.isEmpty()){
             ...
             System.out.println("No hi ha accessos registrats");
         }
         return longNivell;
     }
}